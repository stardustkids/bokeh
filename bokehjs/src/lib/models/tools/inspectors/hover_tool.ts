...
import {div, span, display, undisplay} from "core/dom"
...
export class HoverToolView extends InspectToolView {
  ...
  _update([renderer_view, {geometry}]: [RendererView, {geometry: PointGeometry | SpanGeometry}]): void {
    ...
    if (tooltips.length == 0)
      ...
      const position = tooltips[tooltips.length-1]
      const content = div({}, tooltips.map(([,, content]) => content))
      tooltip.setv({content, position}, {check_eq: false}) // XXX: can't compare DOM nodes
    }
  }
